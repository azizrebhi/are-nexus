...
const AutonomDashboard = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      setLoading(true);
      const { data, error } = await supabase.from('autonomus').select('*');
      if (error) {
  ...
  return (
    <div className="min-h-screen  text-white px-6 sm:px-10 py-10">
      ...
      <h2 className="text-3xl sm:text-5xl text-center my-8 tracking-wide">
        Autonomus <span className="text-[#E2AC0D]">Challlange</span>
      </h2>
...
export default AutonomDashboard;
